...
const MemberCard = ({ member }) => {
  ...
  const getBrandedColor = (name) => {
    if (!name) return '#fb8c01'; // Default color if name is undefined
  ...
  const getInitials = (name) => {
    if (!name) return 'N/A'; // Default initials if name is undefined
        ...
        <div className="flex items-start mb-4 gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-lg font-semibold text-gray-800 mb-1" title={member.organizationName}>
              {member.organizationName || 'Unnamed Organization'}
            </h3>
